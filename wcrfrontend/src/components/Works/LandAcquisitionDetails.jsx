...
export default function LandAcquisitionDetails() {
  ...
  useEffect(() => {
    ...
    saveRouteTitle(
      location.pathname,
	  id ? `Land Acquisition Details - ${id}` : "Land Acquisition Details"
    );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const url = await fetchTrustedTicketUrl(
        ...
        "/views/LADetailsDashboard_17665491649300/LA-Details?:showAppBanner=false&:display_count=n&:showVizHome=n&:origin=viz_share_link&:toolbar=no&:tabs=no"
      );
      setIframeSrc(url);
    };
