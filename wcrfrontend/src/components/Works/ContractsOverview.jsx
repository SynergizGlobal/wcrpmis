...
export default function ContractsOverview() {
  ...
  useEffect(() => {
    ...
    saveRouteTitle(
      location.pathname,
      id ? `Contracts Overview - ${id}` : "Contracts Overview"
    );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const url = await fetchTrustedTicketUrl(
        "contracts-overview",
        "/views/Contract_engineering/2_EngineeringExe?:showAppBanner=false&:display_count=n&:showVizHome=n&:origin=viz_share_link&:toolbar=no&:tabs=no"
      );
      setIframeSrc(url);
    };
