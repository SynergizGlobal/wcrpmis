...
export default function ContractsOverviewInProgress() {
  ...
  useEffect(() => {
    ...
    const id =
      last && last !== "contracts-overview-inprogress"
        ? last
    ...
    saveRouteTitle(
      location.pathname,
      id ? `Contracts In Progress - ${id}` : "Contracts In Progress"
    );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const url = await fetchTrustedTicketUrl(
        "contracts-overview-inprogress",
        "/views/ContractInProgress/InProgressContract?:showAppBanner=false&:display_count=n&:showVizHome=n&:origin=viz_share_link&:toolbar=no&:tabs=no"
      );
      setIframeSrc(url);
    };
