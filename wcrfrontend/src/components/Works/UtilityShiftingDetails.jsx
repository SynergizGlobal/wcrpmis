...
export default function UtilityShiftingDetails() {
  ...
  useEffect(() => {
    ...
    saveRouteTitle(
      location.pathname,
      id ? `Utility Shifting Details - ${id}` : "Utility Shifting Details"
    );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const url = await fetchTrustedTicketUrl(
        ...
        "/views/UtilityShiftingDetails_17667291751700/Utilityshiftingdetails?:showAppBanner=false&:display_count=n&:showVizHome=n&:origin=viz_share_link&:toolbar=no&:tabs=no"
      );
      setIframeSrc(url);
    };
