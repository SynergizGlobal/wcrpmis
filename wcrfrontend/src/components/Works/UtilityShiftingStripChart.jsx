...
export default function UtilityShiftingStripchart() {
  ...
  useEffect(() => {
    ...
    saveRouteTitle(
      location.pathname,
	  id ? `Utility Shifting Stripchart - ${id}` : "Utility Shifting Stripchart"
    );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const url = await fetchTrustedTicketUrl(
        ...
        "/views/UtilityShiftingStripchart_17667380516400/UtilityShiftingStripChart?:showAppBanner=false&:display_count=n&:showVizHome=n&:origin=viz_share_link&:toolbar=no&:tabs=no"
      );
      setIframeSrc(url);
    };
