...
export default function UtilityShiftingOverview() {
  ...
  useEffect(() => {
    ...
    const id =
      last && last !== "utility-shifting-overview"
        ? last
    ...
    saveRouteTitle(
      location.pathname,
      id ? `Utility Shifting Overview - ${id}` : "Utility Shifting Overview"
    );
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      const url = await fetchTrustedTicketUrl(
        "utility-shifting-overview",
        "/views/UtilityShiftingoverview/UtilityShiftingOverviewDashboard?:showAppBanner=false&:display_count=n&:showVizHome=n&:origin=viz_share_link&:toolbar=no&:tabs=no"
      );
      setIframeSrc(url);
    };
  ...
  return (
    <Dashboard>
      <div style={{ marginTop: 20, height: "80vh", position: "relative" }}>
        ...
        {iframeSrc && (
          <iframe
            title="Utility Shifting Overview"
            src={iframeSrc}
            style={{
              width: "100%",
              height: "100%",
