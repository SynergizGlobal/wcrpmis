...
export default function LandAcquisitionOverview() {
  ...
  useEffect(() => {
    ...
    saveRouteTitle(
      location.pathname,
      id ? `Land Acquisition Overview - ${id}` : "Land Acquisition Overview"
    );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const url = await fetchTrustedTicketUrl(
        ...
        "/views/LandAcquisitionOverview/LAOverview?:showAppBanner=false&:display_count=n&:showVizHome=n&:origin=viz_share_link&:toolbar=no&:tabs=no"
      );
      setIframeSrc(url);
    };
