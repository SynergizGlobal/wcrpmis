...
export default function ProgressChart() {
  ...
  useEffect(() => {
    ...
    saveRouteTitle(
      location.pathname,
      id ? `Progress Chart - ${id}` : "Progress Chart"
    );
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      const url = await fetchTrustedTicketUrl(
        "progress-chart",
        "/views/ProgressChart_17667284334730/StructureLevel?:showAppBanner=false&:display_count=n&:showVizHome=n&:origin=viz_share_link&:toolbar=no&:tabs=no"
      );
      setIframeSrc(url);
    };
