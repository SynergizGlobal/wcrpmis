...
import com.wcr.wcrbackend.common.DocxTableCreation;
import com.wcr.wcrbackend.service.IssueService;
import com.wcr.wcrbackend.service.IssuesReportService;

import jakarta.servlet.http.HttpSession;

@RestController
@RequestMapping("/api/issues-report")
public class IssuesReportController {

    public static Logger logger = Logger.getLogger(IssuesReportController.class);

    @Autowired
    private IssuesReportService issueService;
    ...
    @PostMapping(
    	...
    	public ResponseEntity<List<Issue>> getHODListInIssuesReport() {
    	    try {
    	        Issue obj = new Issue(); // NO session user
    	        List<Issue> hodList = issueService.getHODListInIssuesReport(obj);
    	        return ResponseEntity.ok(hodList);
    	    } catch (Exception e) {
    	        logger.error("getHODListInIssuesReport", e);
    ...
    @PostMapping(
    	...
    	public ResponseEntity<List<Issue>> getContractsListInIssuesReport() {
    	    try {
    	        Issue obj = new Issue();
    	        obj.setStatus_fk("Closed"); // same as MVC

    	        List<Issue> contractList =
    	            issueService.getContractsListInIssuesReport(obj);

    	        return ResponseEntity.ok(contractList);
    	    } catch (Exception e) {
    	        logger.error("getContractsListInIssuesReport", e);
    	        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
    	    }
    	}
	...
	public ResponseEntity<List<Issue>> getStatusList(@RequestBody Issue obj) {
	    try {
	        List<Issue> list = issueService.getStatusListInIssuesReport(obj);
	        return ResponseEntity.ok(list);
	    } catch (Exception e) {
	        logger.error("getStatusList error", e);
	...
	public ResponseEntity<List<Issue>> getLocationList(@RequestBody Issue obj) {
	    try {
	        List<Issue> list = issueService.getLocationsListInIssuesReport(obj);
	        return ResponseEntity.ok(list);
	    } catch (Exception e) {
	        logger.error("getLocationList error", e);
	        return ResponseEntity.internalServerError().build();
	    }
	}
	...
	public ResponseEntity<List<Issue>> getCategoryList(@RequestBody Issue obj) {
	    try {
	        List<Issue> list = issueService.getCategoriesListInIssuesReport(obj);
	        return ResponseEntity.ok(list);
	    } catch (Exception e) {
	...
	public ResponseEntity<List<Issue>> getTitleList(@RequestBody Issue obj) {
	    try {
	        List<Issue> list = issueService.getTitlesListInIssuesReport(obj);
	        return ResponseEntity.ok(list);
	    } catch (Exception e) {
	        logger.error("getTitleList error", e);
	        return ResponseEntity.internalServerError().build();
	    }
	}

    @PostMapping("/generate-pending")
    public ResponseEntity<byte[]> generatePendingIssuesReport(
            @RequestBody Issue obj,
            HttpSession session
    ) {
        try {

            obj.setStatus_fk("Closed");
            System.out.println("Contract: " + obj.getContract_id_fk());
            System.out.println("Work: " + obj.getWork_id_fk());
            System.out.println("Status: " + obj.getStatus_fk());

            Map<String, Map<String, List<Issue>>> pendingIssues =
                    issueService.getPendingIssues(obj);

            // Create DOCX
            WordprocessingMLPackage wordMLPackage =
                    WordprocessingMLPackage.createPackage(PageSizePaper.A4, true);

            MainDocumentPart mp = wordMLPackage.getMainDocumentPart();
            ObjectFactory factory = Context.getWmlObjectFactory();

            // Your existing utility
            DocxTableCreation.createTableForPendingIssuesReport(
                    wordMLPackage, mp, factory, pendingIssues
            );
            ...
            headers.setContentDisposition(
                    ContentDisposition.attachment()
                            .filename("Pending_Issues_Report.docx")
                            .build()
            );
            System.out.println("Status sent: " + obj.getStatus_fk());
            System.out.println("Pending Issues Map Size: " + pendingIssues.size());

            return new ResponseEntity<>(fileBytes, headers, HttpStatus.OK);

        } catch (Exception e) {
            logger.error("generatePendingIssuesReport", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
	...
	@PostMapping("/generate-issues-summary-report")
	public ResponseEntity<byte[]> generateIssuesSummaryReport(@RequestBody Issue obj) {

	    try {
	        String currentDate = new SimpleDateFormat("yyyy-MM-dd").format(new Date());

	        byte[] fileBytes = buildIssuesSummaryReport(currentDate, obj);

	        HttpHeaders headers = new HttpHeaders();
	        headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
	        headers.setContentDisposition(
	                ContentDisposition.attachment()
	                        .filename("Issues Summary Report - " + currentDate + ".docx")
	                        .build()
	        );
	...
	@GetMapping("/issues-summary-report/{workId}")
	public ResponseEntity<byte[]> issuesSummaryReportByWork(
	        @PathVariable("workId") String workId) {

	    try {

	        Issue obj = new Issue();
	        obj.setWork_id_fk(workId);
	...
	private byte[] buildIssuesSummaryReport(String currentDate, Issue obj) throws Exception {
	    ...
	    DocxTableCreation.createTableForIssuesSummaryReport(
	            wordMLPackage,
	            mp,
	            factory,
	            issuesCounts
