...
@Repository
public class LACategoryDaoImpl implements LACategoryDao{
	@Autowired
	...
	public List<TrainingType> getLACategoryList() throws Exception {
		...
		try {
			String qry ="select la_category from la_category ";
	...
	public boolean addLACategory(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO la_category"
					+ "( la_category) VALUES (:la_category)";
			...
			if(tablesList.size() > 0) {
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select la_category from la_category where la_category NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='la_category' and s.name='dbo'\r\n" + 
	...
	public boolean updateLandAcquisitionCategory(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table la_category NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE la_category SET la_category= :value_new WHERE la_category= :value_old " ;
			...
			String  enableQry =	"Alter Table la_category CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteLandAcquisitionCategory(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from la_category WHERE la_category= :la_category; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
