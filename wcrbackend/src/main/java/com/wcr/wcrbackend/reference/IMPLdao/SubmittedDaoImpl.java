...
public class SubmittedDaoImpl implements SubmittedDao{
	...
	public TrainingType getSubmittedDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry ="select design_status_submit from design_status_submit ";
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as design_status_submit,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select design_status_submit from design_status_submit where design_status_submit NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getDesign_status_submit();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	public boolean addSubmitted(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO design_status_submit"
					+ "( design_status_submit) VALUES (:design_status_submit)";
	...
	public boolean updateSubmitted(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table design_status_submit NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE design_status_submit SET design_status_submit= :value_new WHERE design_status_submit= :value_old " ;
			...
			String  enableQry =	"Alter Table design_status_submit CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteSubmitted(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from design_status_submit WHERE design_status_submit= :design_status_submit; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='design_status_submit' and s.name='dbo'\r\n" + 
