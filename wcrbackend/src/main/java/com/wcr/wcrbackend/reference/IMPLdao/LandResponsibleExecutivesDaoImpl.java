...
public class LandResponsibleExecutivesDaoImpl implements LandResponsibleExecutivesDao{
	...
	public List<TrainingType> getExecutivesDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT  project_id_fk, project_name, STRING_AGG(u.user_name , ',') user_name,STRING_AGG(u.user_id , ',') user_id FROM land_executives re "
...
//		TransactionStatus status = transactionManager.getTransaction(def);
		try {
			...
			String qry3 = "INSERT into land_executives (project_id_fk,executive_user_id_fk) "
...
//		TransactionStatus status = transactionManager.getTransaction(def);
		try {
			NamedParameterJdbcTemplate namedParamJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);	
			if(!StringUtils.isEmpty(obj.getProject_id_fk_old()) && obj.getProject_id_fk_old() != null) {
					
					String conDeleteQry = "DELETE from land_executives where project_id_fk = :project_id_fk_old";		 
					BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
