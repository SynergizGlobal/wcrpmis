...
@Repository
public class ContractorSpecializationsDaoImpl implements ContractorSpecializationsDao{
	...
	public boolean addContractorSpecialization(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO contractor_specialization"
					+ "( contractor_specialization) VALUES (:contractor_specialization)";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='contractor_specialization' and s.name='dbo'\r\n" + 
	...
	public boolean updateContractorSpecialization(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table contractor_specialization NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			for (TrainingType bObj : obj.getdList()) {
				...
				String updateTableQry = "UPDATE "+bObj.getTable_name()+" SET "+bObj.getColumn_name()+" =:contractor_specialization_new WHERE "+bObj.getColumn_name()+"= :contractor_specialization_old " ;
				
				 paramSource = new BeanPropertySqlParameterSource(obj);		 
				 namedParamJdbcTemplate.update(updateTableQry, paramSource);	
			}
			String  enableQry =	"Alter Table contractor_specialization CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
