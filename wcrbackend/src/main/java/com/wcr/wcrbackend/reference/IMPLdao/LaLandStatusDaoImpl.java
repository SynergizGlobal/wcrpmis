...
@Repository
public class LaLandStatusDaoImpl implements LaLandStatusDao{
					...
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select la_land_status from la_land_status where la_land_status NOT IN (?";
						...
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getLa_land_status();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='la_land_status' and s.name='dbo'\r\n" + 
	...
	public boolean addLaStatus(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO la_land_status"
					+ "( la_land_status) VALUES (:la_land_status)";
	...
	public boolean updatelandAcquisitionStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table la_land_status NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			String  enableQry =	"Alter Table la_land_status CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
