...
public class GeneralStatusDaoImpl implements GeneralStatusDao {
					...
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select general_status from general_status where general_status NOT IN (?";
						...
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getGeneral_status();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='general_status' and s.name='dbo'\r\n" + 
	...
	public boolean updateGeneralStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table general_status NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			String  enableQry =	"Alter Table general_status CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
