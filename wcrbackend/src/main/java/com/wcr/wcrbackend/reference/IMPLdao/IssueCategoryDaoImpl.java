...
public class IssueCategoryDaoImpl implements IssueCategoryDao{
	...
	public boolean addIssueCategory(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO issue_category"
			...
			if(tablesList.size() > 0) {
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select category from issue_category where category NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='issue_category' and s.name='dbo'\r\n" + 
	...
	public boolean updateIssueCategory(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table issue_category NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE issue_category SET category= :value_new WHERE category= :value_old " ;
			...
			String  enableQry =	"Alter Table issue_category CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteIssueCategory(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from issue_category WHERE category= :category; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
