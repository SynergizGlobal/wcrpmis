...
public class StageDaoImpl implements StageDao{
	...
	public TrainingType getStageDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry ="select stage from stage ";
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as stage,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select stage from stage where stage NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getStage();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	public boolean addStage(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO stage"
					+ "( stage) VALUES (:stage)";
	...
	public boolean updateStage(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table stage NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE stage SET stage= :value_new WHERE stage= :value_old " ;
			...
			String  enableQry =	"Alter Table stage CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteStage(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from stage WHERE stage= :stage; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='stage' and s.name='dbo'\r\n" + 
