...
import com.wcr.wcrbackend.reference.Idao.BackgroundTypeDao;
...
public class BackgroundTypeDaoImpl implements BackgroundTypeDao{
	...
	public boolean updateBackgroundType(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table bg_type NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			String  enableQry =	"Alter Table bg_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='bg_type' and s.name='dbo'\r\n" + 
			...
			for(TrainingType tName : tablesList) {
				 ...
				 String captilizedName = sb.toString();
