...
@Repository
public class LAFileTypeDaoImpl implements LAFileTypeDao{
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as la_file_type,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select la_file_type from la_file_type where la_file_type NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getLa_file_type();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	public boolean addLAFileType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO la_file_type"
					+ "( la_file_type) VALUES (:la_file_type)";
	...
	public boolean updateLAFileType(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table la_file_type NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE la_file_type SET la_file_type= :value_new WHERE la_file_type= :value_old " ;
			...
			String  enableQry =	"Alter Table la_file_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteLAFileType(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from la_file_type WHERE la_file_type= :la_file_type; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='la_file_type' and s.name='dbo'\r\n" + 
