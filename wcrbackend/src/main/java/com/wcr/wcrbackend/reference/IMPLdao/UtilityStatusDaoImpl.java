...
import com.wcr.wcrbackend.reference.Idao.UtilityStatusDao;
import com.wcr.wcrbackend.reference.model.Safety;

@Repository
public class UtilityStatusDaoImpl implements UtilityStatusDao{
	...
	public boolean addUtilityStatus(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO utility_status"
					+ "( status) VALUES (:utility_status)";
	...
	private List<Safety> getTablesList(Safety obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='utility_status' and s.name='dbo'\r\n" + 
	...
	public boolean updateUtilityStatus(Safety obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table utility_status NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			String  enableQry =	"Alter Table utility_status CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public Safety getUtilityStatusList(Safety obj) throws Exception {
		...
		Safety sObj =null;
		try {
			String qry ="select status from utility_status ";
