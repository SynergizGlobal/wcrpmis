...
@Repository
public class DrawingTypeDaoImpl implements DrawingTypeDao{
	...
	public List<TrainingType> getDrawingTypesList() throws Exception {
		...
		try {
			String qry ="select drawing_type from drawing_type ";
	...
	public boolean addDrawingType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO drawing_type"
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as drawing_type,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select drawing_type from drawing_type where drawing_type NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getDrawing_type();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='drawing_type' and s.name='dbo'\r\n" + 
	...
	public boolean updateDrawingType(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table drawing_type NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE drawing_type SET drawing_type= :value_new WHERE drawing_type= :value_old " ;
			...
			String  enableQry =	"Alter Table drawing_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteDrawingType(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from drawing_type WHERE drawing_type= :drawing_type; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
