...
public class InsuranceTypeDaoImpl implements InsuranceTypeDao{
	...
	public boolean addInsuranceType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO insurance_type"
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as insurance_type,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select insurance_type from insurance_type where insurance_type NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getInsurance_type();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='insurance_type' and s.name='dbo'\r\n" + 
	...
	public boolean updateInsuranceTypes(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table insurance_type NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE insurance_type SET insurance_type= :value_new WHERE insurance_type= :value_old " ;
			...
			String  enableQry =	"Alter Table insurance_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteInsuranceTypes(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from insurance_type WHERE insurance_type= :insurance_type; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
