...
@Repository
public class ApprovalAuthorityDaoImpl implements ApprovalAuthorityDao{
	@Autowired
					...
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select approval_authority from approval_authority where approval_authority NOT IN (?";
						...
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getApproval_authority();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	public boolean addApprovalAuthority(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO approval_authority"
	...
	public boolean updateApprovalAuthority(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table approval_authority NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			String  enableQry =	"Alter Table approval_authority CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='approval_authority' and s.name='dbo'\r\n" + 
