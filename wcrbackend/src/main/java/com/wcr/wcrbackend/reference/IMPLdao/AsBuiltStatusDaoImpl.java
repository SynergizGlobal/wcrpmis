...
@Repository
public class AsBuiltStatusDaoImpl implements AsBuiltStatusDao{
	@Autowired
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as as_built_status,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select as_built_status from as_built_status where as_built_status NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getAs_built_status();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='as_built_status' and s.name='dbo'\r\n" + 
	...
	public boolean addAsBuiltStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO as_built_status"
					+ "( as_built_status) VALUES (:as_built_status)";
	...
	public boolean updateAsBuiltStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table as_built_status NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE as_built_status SET as_built_status= :value_new WHERE as_built_status= :value_old " ;
			...
			String  enableQry =	"Alter Table as_built_status CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteAsBuiltStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from as_built_status WHERE as_built_status= :as_built_status; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
