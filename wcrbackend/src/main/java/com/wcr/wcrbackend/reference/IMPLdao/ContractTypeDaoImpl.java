...
import com.wcr.wcrbackend.reference.Idao.ContractTypeDao;
...
public class ContractTypeDaoImpl implements ContractTypeDao{
	...
	public List<TrainingType> getContractTypesList() throws Exception {
		...
		try {
			String qry ="select contract_type from contract_type ";
	...
	public boolean addContractType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO contract_type"
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as contract_type,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select contract_type from contract_type where contract_type NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getContract_type();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='contract_type' and s.name='dbo'\r\n" + 
	...
	public boolean updateContractType(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table contract_type NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE contract_type SET contract_type= :contract_type_new WHERE contract_type= :contract_type_old " ;
			paramSource = new BeanPropertySqlParameterSource(obj);		 
			count = namedParamJdbcTemplate.update(updatereferenceTableQry, paramSource);	
			
			for (TrainingType bObj : obj.getdList()) {
				
				String updateTableQry = "UPDATE "+bObj.getTable_name()+" SET "+bObj.getColumn_name()+" =:contract_type_new WHERE "+bObj.getColumn_name()+"= :contract_type_old " ;
				
				 paramSource = new BeanPropertySqlParameterSource(obj);		 
				 namedParamJdbcTemplate.update(updateTableQry, paramSource);	
			}
			String  enableQry =	"Alter Table contract_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteContractType(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE FROM contract_type WHERE contract_type= :contract_type; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
