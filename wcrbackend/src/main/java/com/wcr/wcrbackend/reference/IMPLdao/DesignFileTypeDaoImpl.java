...
@Repository
public class DesignFileTypeDaoImpl implements DesignFileTypeDao{
	...
	public TrainingType getDesignFileTypeDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry ="select design_file_type from design_file_type ";
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as design_file_type,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select design_file_type from design_file_type where design_file_type NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getDesign_file_type();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	public boolean addDesignFileType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO design_file_type"
					+ "( design_file_type) VALUES (:design_file_type)";
	...
	public boolean updateDesignFileType(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table design_file_type NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE design_file_type SET design_file_type= :value_new WHERE design_file_type= :value_old " ;
			...
			String  enableQry =	"Alter Table design_file_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteDesignFileType(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from design_file_type WHERE design_file_type= :design_file_type; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='design_file_type' and s.name='dbo'\r\n" + 
