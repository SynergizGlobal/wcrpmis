...
public class DocumentTypeDaoImpl implements DocumentTypeDao{
	...
	public boolean addDocumentType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO document_type"
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='document_type' and s.name='dbo'\r\n" + 
	...
	public boolean updateDocumentType(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table document_type NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			for (TrainingType bObj : obj.getdList()) {
				...
				String updateTableQry = "UPDATE "+bObj.getTable_name()+" SET "+bObj.getColumn_name()+" =:document_type_new WHERE "+bObj.getColumn_name()+"= :document_type_old " ;
				
				 paramSource = new BeanPropertySqlParameterSource(obj);		 
				 namedParamJdbcTemplate.update(updateTableQry, paramSource);	
			}
			String  enableQry =	"Alter Table document_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
